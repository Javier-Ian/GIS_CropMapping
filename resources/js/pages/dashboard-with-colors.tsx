            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
                ...
                <div className="space-y-4">
                    ...
                    {maps.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            {maps.map((map) => (
                                <Card key={map.id} className="border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 hover:from-green-100 hover:to-emerald-100 transition-all duration-200">
                                    <CardContent className="p-6">
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1">
                                                <div className="mb-2 flex items-center gap-2">
                                                    <h3 className="text-lg font-semibold text-green-800">{map.title}</h3>
                                                ...
                                                <p className="mb-4 text-green-700">{map.description}</p>
                                                <div className="flex items-center gap-4 text-sm text-green-600">
                                                    <span>Created: {formatDate(map.created_at)}</span>
                                                    <span>•</span>
                                                    <span>Files: {map.gis_file_paths?.length || 0}</span>
                                                    {map.gis_file_paths && map.gis_file_paths.length > 0 && (
                                                        <>
                                                            <span>•</span>
                                                            <span>
                                                                Size: {formatFileSize(map.gis_file_paths.reduce((total, file) => total + file.size, 0))}
                                                            </span>
                                                        </>
                                                    )}
                                                </div>
                                            ...
                                            <div className="ml-4 flex items-center gap-2">
                                                {map.map_image_url && (
                                                    <div className="h-20 w-20 overflow-hidden rounded-lg border-2 border-green-200">
                                                        <img src={map.map_image_url} alt={map.title} className="h-full w-full object-cover" />
                                                    </div>
                                                )}
                                                <DropdownMenu>
